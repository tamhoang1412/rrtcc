...
class delay_based_controller:
    ...
    def estimate_bandwidth(self, env, rtcp_timestamp, packets_info, m, gamma_1, last_received_RTP, RTCP_limit, RTP_packet_size):
        i = last_received_RTP
        last_m = m[i]
        second_last_m = m[i]
        last_gamma_1 = gamma_1[-1]
        last_arr_time, second_last_arr_time = packets_info[i][2], packets_info[i-1][2]
        self.new_gamma_1(last_received_RTP, last_m, last_gamma_1, last_arr_time, second_last_arr_time)
        self.state = self.update_state(self.state, last_gamma_1, last_m, second_last_m)
        ...
        if self.state == 'I':
            ...
            if increase_state == "multiplicative":
                ...
                rtt_ms = 2 * (packets_info[last_received_RTP][2] - self.packets_info[last_received_RTP][1])*1000
    ...
    def new_gamma_1(self, last_received_RTP, last_m, last_gamma_1, last_arr_time, second_last_arr_time):
        #last_m = m[last_received_RTP]
        #last_arrival_time - bf_last_arrival_time = self.packets_info[i][2]-self.packets_info[i-1][2]
        if abs(last_m) - last_gamma_1 < 15:
            if abs(last_m) > self.gamma_1[-1]:
                return last_gamma_1 + (last_arr_time - second_last_arr_time) * self.K_d * (abs(last_m)-last_gamma_1)
            else:
                return last_gamma_1 + (last_arr_time - second_last_arr_time) * self.K_u * (abs(last_m)-last_gamma_1)

    def update_state(self, current_state, last_gamma_1, last_m, second_last_m):
        signal = self.get_signal(last_gamma_1, last_m, second_last_m)
    ...
    def get_signal(self, last_gamma_1, last_m, second_last_m):
        #last_m, second_last_m = m[self.last_received_RTP], m[self.last_received_RTP - 1]
        if last_m < second_last_m:
            signal = 'U'
        elif last_m > last_gamma_1 or last_m > self.gamma_2:
            signal = 'O'
        elif last_m <  -last_gamma_1:
            signal = 'U'
        else:
            signal = 'N'
        return signal
